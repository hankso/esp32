...
void btdev_status(btmode_t mode) {
    printf("Connectable: %s, discoverable: %s, evtgrp: 0b%s\n",
        ...
        format_binary(getBits(0xFF), 1));
#ifdef CONFIG_BASE_BT_HID_DEVICE
    if (mode == BT_HIDD) {
        if (!ctx.enabled) {
            puts("App not registered");
        } else {
            printf("App: %s\n", ctx.app.description);
        }
        if (!ctx.connected) {
        ...
        printf("Connected to " BDASTR, BDA2STR(ctx.addr));
        if (strlen(ctx.name)) printf(" (%s)", ctx.name);
        putchar('\n');
...
#else
        if (ptr->bt.cod) printf("0b%s", format_binary(ptr->bt.cod, 2));
#endif
        putchar('\n');
    }
}
...
static void bt_hidd_cb(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param) {
    if (event == ESP_HIDD_INIT_EVT) {
        ...
        ctx.app = (esp_hidd_app_param_t) {
            .name = "BT HID Device",
            .description = "Keyboard(1) + Mouse(2) + Dial(3)",
            .subclass = ESP_HID_CLASS_COM,
            .provider = hid_desc_vendor(NULL),
            .desc_list = (uint8_t *)hid_descriptor_report,
            .desc_list_len = hid_descriptor_report_len,
        };
        ...
        if (param->get_report.report_type != ESP_HIDD_REPORT_TYPE_INPUT) {
            ...
            // FIXME: Currently just sending zeros to the HID host.
            // Maybe we shouldn't reply to GET_REPORT event?
            // e.g. esp_bt_hid_device_report_error(
            //         ESP_HID_PAR_HANDSHAKE_RSP_ERR_UNSUPPORTED_REQ);
            hid_report_t report = {
                .id = param->get_report.report_id, .keybd = { 0 }
            };
            if (bt_send_report(ESP_HIDD_REPORT_TYPE_INPUT, &report))
                esp_bt_hid_device_report_error(
...
static const esp_ble_adv_data_t ble_adv_data = {
    .set_scan_rsp           = false,
    .include_name           = true,
...
static void ble_hidd_cb(void *a, esp_event_base_t b, int32_t id, void *data) {
    ...
    if (id == ESP_HIDD_START_EVENT) {
        ...
        ESP_LOGI(BLE, "Send report %s id 0x%02X size %d",
    ...
    esp_hid_raw_report_map_t ble_report_maps[] = {
        {
            ...
            .len = hid_descriptor_report_len,
        }
    };
    const esp_hid_device_config_t ble_hidd_conf = {
        .vendor_id          = 0x16C0,
        .product_id         = 0x05DF,
        .version            = hid_desc_version(NULL),
        .manufacturer_name  = hid_desc_vendor(NULL),
        .serial_number      = hid_desc_serial(NULL),
        .report_maps        = ble_report_maps,
        .report_maps_len    = 1,
    };
    if (!err)
        err = esp_ble_gap_config_adv_data((esp_ble_adv_data_t *)&ble_adv_data);
    if (!err)
        err = esp_ble_gatts_register_callback(esp_hidd_gatts_event_handler);
    if (!err)
        err = esp_hidd_dev_init(&ble_hidd_conf, ESP_HID_TRANSPORT_BLE,
